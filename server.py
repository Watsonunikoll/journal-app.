...
# Оценки только для конкретного учителя и предмета (Изоляция)
@app.get("/api/grades")
...
# Сохранение оценки с блокировкой
@app.post("/api/grades")
def save_grade(req: GradeSave, db: Session = Depends(get_db)):
    ...
    if existing:
        if existing.is_locked == 1 and existing.teacher_id != req.teacher_id:
            raise HTTPException(status_code=403, detail="Оценка заблокирована для редактирования")
...
# Режим Куратора: Ընդհանուր գնահատականներ (Все оценки)
...
# Скачивание отчёта в Excel для Куратора
...
# АДМИНКА: Обновление названия курса
...
# АДМИНКА: Создание Учителя с привязкой предметов
